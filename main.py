...
from scrapers.venturebeat import scrape_venturebeat
...
def format_news_for_telegram(text: str) -> str:
    """
    Format the generated news for Telegram.
    The first line is displayed as a bold headline.
    """
    ...
    # Remove Markdown bold markers if Gemini already added them
    if (
        headline.startswith("**")
        and headline.endswith("**")
    ):
...
def remove_hashtags(text: str) -> str:
    """
    Remove hashtag lines generated by the AI.
    Hashtags are generated separately by the application.
    """
...
def build_news_text(
    ...
    """
    Build the final English Telegram news format.
    """

    cleaned_ai_text = remove_hashtags(
        ai_text
    )

    formatted_news = format_news_for_telegram(
        cleaned_ai_text
    )
...
def generate_hashtags(text: str) -> str:
    """
    Generate concise English technology hashtags.
    """
    ...
    keywords = {
        ...
        "drones": "#Drones",
        "robotaxi": "#Robotaxi",
        "autonomous": "#AutonomousTechnology",
...
async def check_and_process_news(app_bot) -> None:
    ...
    try:
        ...
        if not is_gemini_available():
            print(
                "[QUEUE] Gemini unavailable. "
                "Skipping queue processing."
            )
        ...
        diagnostic["venturebeat"] = 0
        diagnostic["gemini_quota_blocked"] = False
        ...
        try:
            ...
            vb_articles = scrape_venturebeat(
                max_articles=5
            )

            diagnostic["venturebeat"] = (
                len(vb_articles)
            )

            print(
                f"[SCRAPER] VentureBeat -> "
                f"{len(vb_articles)} article(s)"
            )
        ...
        while True:
            ...
            try:
                ...
                print(
                    "[QUEUE] Gemini quota exceeded. "
                    "Stopping current queue cycle."
                )
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "🚨 Publish as Breaking",
                        callback_data=f"urgent_{preview_news_id}"
                    ),
                    InlineKeyboardButton(
                        "✅ Publish",
                        callback_data=f"normal_{preview_news_id}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        "✏️ Edit News",
                        callback_data=f"edit_{preview_news_id}"
                    ),
                    InlineKeyboardButton(
                        "❌ Reject",
                        callback_data=f"skip_{preview_news_id}"
                    )
                ]
            ]
            ...
            preview_text = (
                f"📰 <b>News Preview ({html.escape(source)})</b>\n\n"
                f"{build_news_text(ai_text, url)}"
            )
    ...
    try:
        ...
        if user_id in user_editing_state:
            ...
            if news_id in pending_news:
                ...
                preview_msg = (
                    f"📝 <b>Edited News Preview</b>\n\n"
                    f"{safe_text}\n\n"
                    f"<b>Source:</b>\n"
                    f"{html.escape(pending_news[news_id]['url'])}"
                )
    ...
    try:
        ...
        if data == "toggle_status":
            ...
            report = (
                ...
                f"VentureBeat: {diagnostic.get('venturebeat', 0)}\n\n"
